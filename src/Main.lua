...
callbacks.Register('Draw', 'Draw-F3drQ', function()
    ...
    elements[1] = { 'Crit',
        ...
        colors.gray }
    elements[2] = { (weaponinfo.shots_left_till_bucket_full or 0) .. ' attacks left until full bar', nil,
                    (weaponinfo.shots_left_till_bucket_full or 0) ~= 0 }
    elements[3] = { (weaponinfo.stored_crits or 0) .. ' crits available' }
    elements[4] = { 'deal ' .. math.floor(other_weapon_info.requiredDamage or 0) .. ' damage', nil,
                    (other_weapon_info.critChance or 0) + 0.1 < (other_weapon_info.observedCritChance or 0) }
...
callbacks.Register('CreateMove', 'CreateMove-N8bat', function()
    ...
    if needupdate == nil or wpn:GetIndex() ~= (weaponinfo and weaponinfo.currentWeapon) then
        ...
        weaponinfo.shots_to_fill_bucket           = weaponinfo.bucket_max / (weaponinfo.added_per_shot or 1)
        weaponinfo.costs                          = {}
        ...
        if wpn:IsMeleeWeapon() then
            ...
            for i = 0, (weaponinfo.shots_to_fill_bucket or 0) + 1, 1 do
    ...
    other_weapon_info.requiredDamage = requiredTotalDamage - (damageStats['total'] or 0)
